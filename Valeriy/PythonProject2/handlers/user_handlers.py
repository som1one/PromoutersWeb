...
from handlers.validation import FormValidator
...
class UserHandlers:
    ...
    def _register_message_handlers(self):
        ...
        @self.bot.message_handler(func=lambda m: m.text in ("💼 Касса", "Касса"))
        def city_cash(message: types.Message):
            self.director_handler.handle_cash_overview(message)
    ...
    def _register_callback_handlers(self):
        ...
        @self.bot.callback_query_handler(func=lambda c: c.data == "transfer_to_director")
        def transfer_to_director_callback(callback: types.CallbackQuery):
            self._handle_transfer_to_director(callback)
        
        @self.bot.callback_query_handler(func=lambda c: c.data.startswith("select_director:"))
        def select_director_callback(callback: types.CallbackQuery):
            self._handle_select_director(callback)
        ...
        @self.bot.callback_query_handler(func=lambda c: c.data == "sd_zero")
        def sd_zero_callback(callback: types.CallbackQuery):
            self.order_handler.handle_sd_zero_callback(callback)
        ...
        @self.bot.callback_query_handler(func=lambda c: c.data == "cash_clear")
        def cash_clear_callback(callback: types.CallbackQuery):
            self.director_handler.handle_cash_clear(callback)
        
        @self.bot.callback_query_handler(func=lambda c: c.data == "cash_clear_confirm")
        def cash_clear_confirm_callback(callback: types.CallbackQuery):
            self.director_handler.handle_cash_clear_confirm(callback)
        ...
        @self.bot.callback_query_handler(func=lambda c: c.data.startswith("view_receipt:"))
        def view_receipt_callback(callback: types.CallbackQuery):
            self.master_handler.handle_view_receipt(callback)
    ...
    def _handle_setrole(self, message: types.Message):
        ...
        try:
            ...
            tg_id = int(parts[1])
            newrole = parts[2].lower()
            allowed_roles = {"owner", "director", "dispatcher", "master", "user"}
            if newrole not in allowed_roles:
                self.bot.reply_to(message, "❌ Неверная роль. Доступные: owner, director, dispatcher, master, user")
                return

            # Для директора требуется выбрать город
            if newrole == "director":
                ...
                self.bot.reply_to(message, "📍 Выберите город для директора:", reply_markup=kb)
                return

            # Иначе — применяем роль сразу
            app_user = session.query(User).filter_by(tg_id=tg_id).first()
            if not app_user:
                app_user = User(tg_id=tg_id, name=str(tg_id))
                session.add(app_user)
                session.flush()
            app_user.role = newrole
            # city_id оставляем без изменений; при необходимости настроим позже
            session.commit()
            db_optimizer.invalidate_user_cache(tg_id)

            # Если меняем роль себе — сразу обновим меню
            if tg_id == message.from_user.id:
                kb = main_keyboard(newrole)
                self.bot.send_message(message.chat.id, f"✅ Ваша роль изменена на: {newrole}", reply_markup=kb)
            else:
                self.bot.reply_to(message, f"✅ Роль пользователя {tg_id} изменена на: {newrole}")
        except Exception as e:
    ...
    def _handle_edit_order_field(self, callback: types.CallbackQuery):
        ...
        try:
            ...
            if field == "city":
                ...
                # Переключаем гарантию (без ввода текста)
                data = state.get("data") or {}
                current = bool(data.get("is_warranty", False))
                new_val = not current
                data["is_warranty"] = new_val
                state["data"] = data
                if state.get("order_data"):
                    state["order_data"]["is_warranty"] = new_val
                self.bot.answer_callback_query(callback.id, f"Гарантия: {'ДА' if new_val else 'НЕТ'}")
                # Перерисуем превью
                self.form_handler._return_to_preview(callback.message, user_id, session)
                return
            ...
            if method == "by_id":
                ...
                if step == "tg_id":
                    try:
                        ...
                        result_text = (
                            f"✅ <b>Мастер успешно создан по Telegram ID!</b>\n\n"
                            f"🆔 Telegram ID: {tg_id}\n"
                            f"👤 Имя: Мастер {tg_id}\n"
                            f"📱 Телефон: Не указан\n"
                            f"🏙 Город: Не указан\n"
                            f"🔧 Роль: Мастер\n\n"
                            f"💡 <i>Мастер может обновить свою информацию через /start</i>"
                        )
                        role = db_optimizer.get_role_cached(user_id)
                        kb = main_keyboard(role)
                ...
                if step == "full_name":
                    ...
                    try:
                        ...
                        result_text = (
                            f"✅ <b>Мастер создан!</b>\n\n"
                            f"👤 {state['data']['full_name']}\n"
                            f"📱 {state['data']['phone']}\n"
                            f"🆔 {tg_id}\n"
                            f"🏙 ID города: {state['data']['city_id']}\n"
                            f"🔧 Мастер"
                        )
    ...
    def _handle_transfer_to_director(self, callback: types.CallbackQuery):
        """Обработать передачу заявки директору"""
        session = get_session()
        try:
            user_id = callback.from_user.id
            state = self.states.get_form_state(user_id)
            ...
            # Получаем город заявки
            order_city_id = state["data"].get("city_id")
            if not order_city_id:
                self.bot.answer_callback_query(callback.id, "Сначала выберите город для заявки")
                return
            
            # Получаем директоров только для этого города
            directors = session.query(User).filter_by(role="director", city_id=order_city_id).all()
            if not directors:
                self.bot.answer_callback_query(callback.id, "Нет директоров для этого города")
                return
            
            # Показываем список директоров
            from handlers.menu_kb import director_selection_keyboard
            kb = director_selection_keyboard(directors)
            self.bot.send_message(callback.message.chat.id, "👔 Выберите директора для передачи заявки:", reply_markup=kb)
            self.bot.answer_callback_query(callback.id, "Выберите директора")
        except Exception as e:
            self.error_handler.handle_general_error_callback(callback, e, "transfer_to_director")
        finally:
            session.close()
    
    def _handle_select_director(self, callback: types.CallbackQuery):
        """Обработать выбор директора для передачи заявки"""
        session = get_session()
        try:
            user_id = callback.from_user.id
            state = self.states.get_form_state(user_id)
            ...
            state["data"]["assigned_to"] = director_tg_id
            
            # Получаем информацию о директоре
            director = session.query(User).filter_by(tg_id=director_tg_id).first()
            if director:
                director_name = director.full_name or director.name or f"ID {director_tg_id}"
                director_info = f"{director_name}"
                if director.city_rel:
                    director_info += f" ({director.city_rel.name})"
            else:
                director_name = f"ID {director_tg_id}"
                director_info = director_name
            
            self.bot.edit_message_text(
                f"✅ Директор {director_info} выбран!", 
            ...
            self.bot.answer_callback_query(callback.id, f"Директор {director_info} выбран")
        except Exception as e:
            self.error_handler.handle_general_error_callback(callback, e, "select_director")
        finally:
            session.close()
